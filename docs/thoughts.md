...
This first version analyzes the full thought on demand. It does not invoke a model in the background, perform incremental scheduling, infer factual continuation from timestamps, or offer drag-and-drop graph editing. Graph edges are backed by their evidence cards below. Confirmed links and suggestions remain local until the user supplies the selected source material to their AI.

Use Add relation to select two conversations, quote their original text and describe the directional relationship. Saving creates a confirmed manual relation. Edit relation can revise an AI suggestion or an existing relationship; concurrent edits are rejected. Source excerpts are expandable in the editor. Dismiss removes a relationship from the graph without deleting its source conversations.
