...
The App/Dock icon uses the approved sage-green transparent artwork in `web/assets/threadline-app-icon.png`. Installation builds `ThreadlineGreen.icns` and startup explicitly applies this bundle icon so a running Dock instance refreshes its artwork. The About panel explicitly receives this same image instead of relying on the system's cached application icon. Web icon references carry a sage artwork revision to invalidate older browser caches.

The menu bar uses an independent hollow stem with a solid ribbon vector, generated by `scripts/make-menu-icon.swift` as `MenuIcon.pdf`. Its alpha has no background tile. The contour uses a 6.2-unit stroke on a 100-unit canvas for legibility at menu-bar size. `isTemplate = true` lets macOS render it white on dark menu bars and dark on light menu bars. Rebuild both resources with `scripts/install-mac.sh`.

Claude Code, Pi and DeepSeek Harness currently use locally rendered C, π and D letter tiles in message headers. They do not reuse another runtime’s logo or load remote assets.
