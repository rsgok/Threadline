...
final class AppDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate, WKNavigationDelegate, WKUIDelegate, WKDownloadDelegate, WKScriptMessageHandler, NSToolbarDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        if let url=Bundle.main.url(forResource:"TrayIcon",withExtension:"png"), let image=NSImage(contentsOf:url){image.size=NSSize(width:19,height:19);image.isTemplate=true;tray.button?.image=image}
        let menu=NSMenu()
        ...
        menu.addItem(.separator());menu.addItem(withTitle:"退出 Threadline",action:#selector(NSApplication.terminate(_:)),keyEquivalent:"")
        for item in menu.items {item.target=self};tray.menu=menu
        ...
        window.toolbar=nil
        let config=WKWebViewConfiguration();config.userContentController.add(self,name:"openInCodex")
    ...
    func toolbar(_ toolbar:NSToolbar,itemForItemIdentifier id:NSToolbarItem.Identifier,willBeInsertedIntoToolbar flag:Bool)->NSToolbarItem?{
        ...
        let options:[String:(String,String,Selector)]=["captureProgress":("留下进展","plus.bubble",#selector(progress)),"library":("资料库","books.vertical",#selector(library)),"search":("搜索","magnifyingglass",#selector(search)),"cursor":("Cursor Agents","link",#selector(openInCursor)),"codex":("在 Codex 打开","sidebar.right",#selector(openInCodex))]
    ...
    @objc func search(){library();web.evaluateJavaScript("goWorkspace('all').then(()=>{document.getElementById(isPanel()?'panel-search':'search').focus()})")}
    ...
    @objc func about(){NSApp.orderFrontStandardAboutPanel(options:[.applicationName:"Threadline · 思续",.applicationVersion:"0.3.0",.credits:NSAttributedString(string:"把想清楚的，带到下一次。")])}
